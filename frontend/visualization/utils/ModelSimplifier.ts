 ...
 */
export enum SimplificationMethod {
  /** Vertex reduction */
...
export interface SimplificationOptions {
  ...
  /** Target percentage (0.0 - 1.0) of original detail to retain */
  ...
  /** Voxel size (for voxel-based simplification) */
  voxelSize?: number;
...
export class ModelSimplifier {
   ...
   */
  constructor(options: SimplificationOptions = {}) {
    this.options = {
      ...defaultOptions,
      ...options,
    };
  ...
  simplifyGeometry(
    ...
    // Merge options
    const mergedOptions: SimplificationOptions = {
      ...this.options,
      ...options,
    };
    ...
    if (!clonedGeometry.index) {
      clonedGeometry.setIndex(
        BufferGeometryUtils.mergeVertices(clonedGeometry).index
      );
    }
   ...
   * @returns Simplified geometry
   */
  private simplifyByVertexReduction(
    geometry: THREE.BufferGeometry,
    options: SimplificationOptions
  ): THREE.BufferGeometry {
    ...
    const targetVertexCount = Math.max(
      4,
      Math.floor(originalVertexCount * (options.targetPercentage || 0.5))
    );
   ...
   * @returns Simplified geometry
   */
  private simplifyByFaceReduction(
    geometry: THREE.BufferGeometry,
    options: SimplificationOptions
  ): THREE.BufferGeometry {
    // Ensure geometry has indices
    ...
    const targetFaceCount = Math.max(
      1,
      Math.floor(originalFaceCount * (options.targetPercentage || 0.5))
    );
   ...
   * @returns Simplified geometry
   */
  private simplifyByMeshDecimation(
    geometry: THREE.BufferGeometry,
    options: SimplificationOptions
  ): THREE.BufferGeometry {
    // Ensure geometry has indices
    ...
    if (options.targetTriangles) {
      ...
      targetTriangleCount = Math.max(
        4,
        Math.floor(originalTriangleCount * (options.targetPercentage || 0.5))
      );
    }
   ...
   * @returns Simplified geometry
   */
  private simplifyByVoxelization(
    geometry: THREE.BufferGeometry,
    options: SimplificationOptions
  ): THREE.BufferGeometry {
    // Get position attribute
    ...
    const voxelSize = options.voxelSize || Math.max(
      (boundingBox.max.x - boundingBox.min.x),
      (boundingBox.max.y - boundingBox.min.y),
      (boundingBox.max.z - boundingBox.min.z)
    ) * (1 - (options.targetPercentage || 0.5));
    ...
    // Create voxel grid
    const voxelGrid = new Map<string, { position: THREE.Vector3, normal: THREE.Vector3, uv?: THREE.Vector2, color?: THREE.Color }>();
    ...
    for (let i = 0; i < positionAttribute.count; i++) {
      ...
      // Create voxel data
      const voxelData: { position: THREE.Vector3, normal: THREE.Vector3, uv?: THREE.Vector2, color?: THREE.Color } = {
        position: new THREE.Vector3(
          (voxelX + 0.5) * voxelSize,
          (voxelY + 0.5) * voxelSize,
          (voxelZ + 0.5) * voxelSize
        ),
      ...
      if (options.preserveTexCoords && geometry.attributes.uv) {
        const uvAttribute = geometry.getAttribute('uv');
        voxelData.uv = new THREE.Vector2(
          uvAttribute.getX(i),
          uvAttribute.getY(i)
        );
      }
      ...
      if (options.preserveColors && geometry.attributes.color) {
        ...
        if (colorAttribute.itemSize === 3) {
          voxelData.color = new THREE.Color(
   ...
   * @returns Simplified geometry
   */
  private simplifyToBoundingBox(geometry: THREE.BufferGeometry): THREE.BufferGeometry {
    // Calculate bounding box
    geometry.computeBoundingBox();
    const boundingBox = geometry.boundingBox!;
   ...
   * @returns Simplified geometry
   */
  private simplifyToBoundingSphere(geometry: THREE.BufferGeometry): THREE.BufferGeometry {
   ...
   * @returns Simplified geometry
   */
  private simplifyToBillboard(geometry: THREE.BufferGeometry): THREE.BufferGeometry {
    // Calculate bounding box
    geometry.computeBoundingBox();
    const boundingBox = geometry.boundingBox!;
  ...
  private simplifyToPointCloud(
    ...
    const targetPointCount = Math.max(
      4,
      Math.floor(originalPointCount * (options.targetPercentage || 0.1))
    );
