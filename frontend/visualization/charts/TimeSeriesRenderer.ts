...
interface ITimeSeriesData {
  /** Data series */
  series: ITimeSeriesDataSeries[];
}

/**
 * Processed time series data
 */
interface IProcessedTimeSeriesData {
  /** Data series */
  series: ITimeSeriesDataSeries[];
 ...
 */
interface ITimeSeriesDimensions {
  /** Container width */
 ...
 */
interface ITimeSeriesAxisLabels {
  /** X axis label */
  x: string;
...
/**
 * Time Series Renderer options
 */
interface ITimeSeriesRendererOptions {
  /** Chart margin */
  margin?: ITimeSeriesMargin;
  ...
  /** Axis labels */
  axisLabels?: ITimeSeriesAxisLabels;
...
class TimeSeriesRenderer extends BaseRenderer {
  /** Last rendered data */
  protected lastData: ITimeSeriesData | null = null;

  /**
   * Create a new TimeSeriesRenderer instance
   * @param options - Renderer options
   */
  constructor(options: ITimeSeriesRendererOptions = {}) {
    super({
      // Default margin
      margin: {
        top: 20,
   ...
   */
  initialize(options: ITimeSeriesRendererOptions = {}): void {
    // Merge options
    this.options = {
      ...this.options,
      ...options,
   ...
   */
  render(element: HTMLElement, data: ITimeSeriesData, options: ITimeSeriesRendererOptions = {}): void {
    // Merge options
    this.options = {
      ...this.options,
      ...options,
   ...
   * @private
   */
  private _processData(data: ITimeSeriesData): IProcessedTimeSeriesData {
    // Extract series
   ...
   * @private
   */
  private _calculateDimensions(element: HTMLElement, data: IProcessedTimeSeriesData): ITimeSeriesDimensions {
   ...
   * @private
   */
  private _createScales(data: IProcessedTimeSeriesData, dimensions: ITimeSeriesDimensions): ITimeSeriesScales {
    // Create x scale
   ...
   * @private
   */
  private _createAxes(svg: SVGElement, scales: ITimeSeriesScales, dimensions: ITimeSeriesDimensions): void {
   ...
   * @private
   */
  private _createGrid(svg: SVGElement, scales: ITimeSeriesScales, dimensions: ITimeSeriesDimensions): void {
   ...
   * @private
   */
  private _createLines(svg: SVGElement, data: IProcessedTimeSeriesData, scales: ITimeSeriesScales, dimensions: ITimeSeriesDimensions): void {
   ...
   * @private
   */
  private _createPoints(svg: SVGElement, data: IProcessedTimeSeriesData, scales: ITimeSeriesScales, dimensions: ITimeSeriesDimensions): void {
   ...
   * @private
   */
  private _createAreas(svg: SVGElement, data: IProcessedTimeSeriesData, scales: ITimeSeriesScales, dimensions: ITimeSeriesDimensions): void {
   ...
   * @private
   */
  private _createLegend(svg: SVGElement, data: IProcessedTimeSeriesData, dimensions: ITimeSeriesDimensions): void {
    ...
    data.series.forEach((series, index) => {
      ...
      itemGroup.setAttribute(
        'transform',
        `translate(${legendX + 10}, ${legendY + 15 + index * 20})`
      );
  ...
  private _addEventListeners(
    svg: SVGElement,
    tooltip: HTMLElement,
    data: IProcessedTimeSeriesData,
    scales: ITimeSeriesScales,
    dimensions: ITimeSeriesDimensions
  ): void {
    // Get all points
