...
/**
 * Pattern heat map data interface
 */
interface IPatternHeatMapData {
  /** Pattern data array */
...
/**
 * Pattern processed data interface
 */
interface IProcessedPatternData {
  /** Pattern names */
  patterns: string[];
...
/**
 * Pattern heat map renderer options
 */
interface IPatternHeatMapRendererOptions {
  /** Cell size in pixels */
  cellSize?: number;
...
class PatternHeatMapRenderer extends BaseRenderer {
   ...
   */
  constructor(options: IPatternHeatMapRendererOptions = {}) {
    super({
      // Default cell size
      cellSize: 20,
   ...
   */
  initialize(options: IPatternHeatMapRendererOptions = {}): void {
    // Merge options
    this.options = {
      ...this.options,
      ...options,
   ...
   */
  render(element: HTMLElement, data: IPatternHeatMapData, options: IPatternHeatMapRendererOptions = {}): void {
    // Merge options
    this.options = {
      ...this.options,
      ...options,
   ...
   * @private
   */
  private _processData(data: IPatternHeatMapData): IProcessedPatternData {
    ...
    // Create matrix
    const matrix: IPatternCell[][] = [];
    const values: number[] = [];

    for (let i = 0; i < patterns.length; i++) {
      const row: IPatternCell[] = [];
   ...
   * @private
   */
  private _generateTimeIntervals(data: IPatternHeatMapData): string[] {
    // Default to 24 hours
   ...
   * @private
   */
  private _calculateDimensions(element: HTMLElement, data: IProcessedPatternData): IDimensions {
    // Get container dimensions
    const containerWidth = element.clientWidth;
    const containerHeight = element.clientHeight;

    // Get margin
    const margin = this.options.margin as { top: number; right: number; bottom: number; left: number };
    ...
    // Calculate cell size
    const cellWidth = Math.min(this.options.cellSize as number, availableWidth / data.timeIntervals.length);
    const cellHeight = Math.min(this.options.cellSize as number, availableHeight / data.patterns.length);
   ...
   * @private
   */
  private _createScales(data: IProcessedPatternData, dimensions: IDimensions): IScales {
    // Create color scale
    const colorScale = (value: number): string => {
      ...
      return colorUtils.interpolateColor(
        (this.options.colorRange as string[])[0],
        (this.options.colorRange as string[])[1],
        normalized
      );
    };
   ...
   * @private
   */
  private _createAxes(svg: SVGElement, scales: IScales, dimensions: IDimensions): void {
  ...
  private _createCells(
    ...
    for (let i = 0; i < data.matrix.length; i++) {
      for (let j = 0; j < data.matrix[i].length; j++) {
        ...
        rect.setAttribute('width', (dimensions.cellWidth - (this.options.padding as number)).toString());
        rect.setAttribute('height', (dimensions.cellHeight - (this.options.padding as number)).toString());
   ...
   * @private
   */
  private _createLegend(svg: SVGElement, scales: IScales, dimensions: IDimensions): void {
   ...
   * @private
   */
  private _createTooltip(container: HTMLElement): HTMLElement {
    // Create tooltip
  ...
  private _addEventListeners(
    ...
    cells.forEach(cell => {
      ...
      cell.addEventListener('mouseenter', event => {
        ...
        // Position tooltip
        const rect = (cell as Element).getBoundingClientRect();
   ...
   * @private
   */
  private _applyTooltipTheme(tooltip: HTMLElement): void {
    // Apply theme based on options
    if (this.options.theme === 'dark') {
...
export { PatternHeatMapRenderer, IPatternHeatMapData, IPatternHeatMapRendererOptions };
