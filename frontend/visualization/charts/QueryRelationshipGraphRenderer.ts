 ...
 * Node type
 */
type NodeType = 'query' | 'pattern' | 'field' | 'operator';
...
interface IGraphLink {
  ...
  source: string | IGraphNode;
  
  /** Target node ID or node object */
  target: string | IGraphNode;
...
/**
 * Query relationship graph data interface
 */
interface IQueryRelationshipGraphData {
  /** Graph nodes */
  nodes: IGraphNode[];
...
/**
 * Processed relationship graph data
 */
interface IProcessedGraphData {
  /** Graph nodes */
  nodes: IGraphNode[];
...
/**
 * Query relationship graph renderer options
 */
interface IQueryRelationshipGraphRendererOptions {
  /** Node size by type */
  nodeSize?: Record<NodeType, number>;
...
class QueryRelationshipGraphRenderer extends BaseRenderer {
   ...
   */
  constructor(options: IQueryRelationshipGraphRendererOptions = {}) {
    super({
      // Default node size
      nodeSize: {
        query: 10,
   ...
   */
  initialize(options: IQueryRelationshipGraphRendererOptions = {}): void {
    // Merge options
    this.options = {
      ...this.options,
      ...options,
   ...
   */
  render(element: HTMLElement, data: IQueryRelationshipGraphData, options: IQueryRelationshipGraphRendererOptions = {}): void {
    // Merge options
    this.options = {
      ...this.options,
      ...options,
   ...
   * @private
   */
  private _processData(data: IQueryRelationshipGraphData): IQueryRelationshipGraphData {
    // Clone data to avoid modifying original
    const nodes: IGraphNode[] = [...data.nodes];
    const links: IGraphLink[] = [...data.links];
   ...
   * @private
   */
  private _createLinks(svg: SVGElement, data: IQueryRelationshipGraphData): SVGElement {
   ...
   * @private
   */
  private _createNodes(svg: SVGElement, data: IQueryRelationshipGraphData): SVGElement {
    // Create group for nodes
    const nodesGroup = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    nodesGroup.setAttribute('class', 'nodes');

    // Get node sizes and colors
    const nodeSize = this.options.nodeSize as Record<NodeType, number>;
    const nodeColors = this.options.nodeColors as Record<NodeType, string>;
   ...
   * @private
   */
  private _createLegend(svg: SVGElement, data: IQueryRelationshipGraphData): void {
   ...
   * @private
   */
  private _addEventListeners(svg: SVGElement, tooltip: HTMLElement, data: IQueryRelationshipGraphData): void {
    // Get all nodes
   ...
   * @private
   */
  private _initializeForceSimulation(
    data: IQueryRelationshipGraphData,
    ...
    const simulation = d3
      ...
      .force(
        ...
        d3
          ...
          .id((d: any) => d.id)
          .distance(this.options.distance as number)
      )
      .force('charge', d3.forceManyBody().strength(this.options.chargeStrength as number))
      .force(
        'center',
        d3.forceCenter(
          nodesGroup.parentNode ? (nodesGroup.parentNode as Element).clientWidth / 2 : 0,
          nodesGroup.parentNode ? (nodesGroup.parentNode as Element).clientHeight / 2 : 0
        )
      );
    ...
    simulation.on('tick', () => {
      ...
      linkElements.forEach((link, i) => {
        const source = data.links[i].source as any;
        const target = data.links[i].target as any;
...
export { 
  QueryRelationshipGraphRenderer, 
  IQueryRelationshipGraphData, 
  IGraphNode, 
  IGraphLink,
  NodeType
};
