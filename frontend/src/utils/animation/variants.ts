 ...
 */
export function createQuantumVariants(options = {}) {
    const { duration = 0.8, ease = "easeOut", delay = 0, scale = [0.8, 1.1, 1], rotation = [0, 5, 0], opacity = [0, 0.7, 1] } = options;
    return {
        hidden: {
            opacity: opacity[0],
            scale: scale[0],
 ...
 */
export function createSlideVariants(direction = 'right', distance = 50, options = {}) {
    const { duration = 0.5, ease = "easeOut", delay = 0 } = options;
    const getOffset = () => {
        switch (direction) {
 ...
 */
export function createFadeVariants(options = {}) {
    const { duration = 0.6, ease = "easeInOut", delay = 0, blur = false } = options;
    return {
        hidden: {
            opacity: 0,
 ...
 */
export function createScaleVariants(options = {}) {
    const { duration = 0.7, ease = "easeOut", delay = 0, scaleFrom = 0.8, scaleTo = 1, overshoot = true } = options;
 ...
 */
export function createRotationVariants(options = {}) {
    const { duration = 1, ease = "linear", delay = 0, rotateFrom = 0, rotateTo = 360, continuous = false } = options;
    return {
        hidden: {
            opacity: 0,
            rotate: rotateFrom,
 ...
 */
export function createPathVariants(options = {}) {
    const { duration = 2, ease = "easeInOut", delay = 0 } = options;
    return {
        hidden: {
            pathLength: 0,
            opacity: 0
