...
interface ConsciousnessStreamDemoProps {
  /** Container width */
  width?: number;
 ...
 * Consciousness Stream Demo Component
 */
const ConsciousnessStreamDemo: React.FC<ConsciousnessStreamDemoProps> = ({
  width = 800,
  height = 600,
  ...
  const testerRef = useRef<ConsciousnessStreamTester | null>(null);
  const monitorRef = useRef<ConsciousnessStreamMonitor | null>(null);
  
  // State
  const [fps, setFps] = useState<number>(0);
  const [testResults, setTestResults] = useState<ConsciousnessStreamTestResult[]>([]);
  const [eventCounts, setEventCounts] = useState<Record<string, number>>({});
  const [continuityScore, setContinuityScore] = useState<number>(1.0);  
  // Initialize scene
  useEffect(() => {
    ...
    testerRef.current = new ConsciousnessStreamTester(
      scene,
      camera,
      renderer,
      {
    ...
    monitorRef.current = new ConsciousnessStreamMonitor(
      testerRef.current,
      {
   ...
   * @returns Color
   */
  const getContinuityScoreColor = (score: number): string => {
    if (score >= 0.9) {
      return '#00ff00'; // Green
    } else if (score >= 0.7) {
      return '#ffff00'; // Yellow
