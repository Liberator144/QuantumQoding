...
import { 
  ...
  LODTransitionSystem,
  TransitionMethod,
  ViewDistanceOptimizer,
} from '../../../visualization/utils';

interface LODDemoProps {
  /** Container width */
  width?: number;
 ...
 * LOD Demo Component
 */
const LODDemo: React.FC<LODDemoProps> = ({
  width = 800,
  height = 600,
  ...
  const modelSimplifierRef = useRef<ModelSimplifier | null>(null);
  const transitionSystemRef = useRef<LODTransitionSystem | null>(null);
  const viewDistanceOptimizerRef = useRef<ViewDistanceOptimizer | null>(null);
  ...
  useEffect(() => {
    ...
    viewDistanceOptimizerRef.current = new ViewDistanceOptimizer(
      lodManagerRef.current,
      transitionSystemRef.current,
      { 
  ...
  const createModels = () => {
    ...
    for (let i = 0; i < modelCount; i++) {
      ...
      // Create LOD model
      const lodModel = modelSimplifierRef.current.createLODModel(
        geometry,
        material,
        5,
        {
