...
import { 
  ...
  OcclusionCullingMethod,
} from '../../../visualization/utils';

interface VirtualRenderingDemoProps {
  /** Container width */
  width?: number;
 ...
 * Virtual Rendering Demo Component
 */
const VirtualRenderingDemo: React.FC<VirtualRenderingDemoProps> = ({
  width = 800,
  height = 600,
  objectCount = 1000,
  enableBufferZones = true,
  ...
  const bufferZoneManagerRef = useRef<BufferZoneManager | null>(null);
  const occlusionCullingSystemRef = useRef<OcclusionCullingSystem | null>(null);
  
  // State
  const [fps, setFps] = useState<number>(0);
  const [visibleObjects, setVisibleObjects] = useState<number>(0);
  const [culledObjects, setCulledObjects] = useState<number>(0);  
  // Initialize scene
  useEffect(() => {
    ...
    if (enableBufferZones) {
      bufferZoneManagerRef.current = new BufferZoneManager(
        scene,
        camera,
        { 
          zoneSize: 20,
    ...
    if (enableOcclusionCulling) {
      occlusionCullingSystemRef.current = new OcclusionCullingSystem(
        scene,
        camera,
        renderer,
        {
