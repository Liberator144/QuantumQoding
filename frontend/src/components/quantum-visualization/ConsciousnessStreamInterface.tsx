...
import { ConsciousnessStreamPacket } from '../../../backend/interdimensional/consciousness/ConsciousnessStreamProtocol';

/**
 * Consciousness stream interface props
 */
interface ConsciousnessStreamInterfaceProps {
  /** Stream ID */
  streamId: string;
  
  /** Stream packets */
  packets: ConsciousnessStreamPacket<any>[];
  ...
  /** Whether to show packet details */
  showPacketDetails?: boolean;
  
  /** Whether to animate stream flow */
  animate?: boolean;
  
  /** Whether to show quantum state information */
  showQuantumState?: boolean;
  
  /** Callback when a packet is clicked */
  onPacketClick?: (packet: ConsciousnessStreamPacket<any>) => void;
  
  /** Callback when a new packet is created */
  onPacketCreate?: (packet: Partial<ConsciousnessStreamPacket<any>>) => void;
 ...
 */
const ConsciousnessStreamInterface: React.FC<ConsciousnessStreamInterfaceProps> = ({
  streamId,
  packets,
  width = 800,
  height = 400,
  ...
  useEffect(() => {
    ...
    // Create time scale
    const timeExtent = d3.extent(sortedPackets, d => d.header.timestamp) as [number, number];
    ...
    // Helper functions
    function getPacketColor(packet: ConsciousnessStreamPacket<any>): string {
  ...
  return (
    <div className={`consciousness-stream-interface ${className || ''}`} style={{ position: 'relative' }}>
      <svg ref={svgRef} />
      <div
        ref={tooltipRef}
        style={{
      ...
      />
      {selectedPacket && (
        <div className="packet-details-panel" style={{
          position: 'absolute',
          top: 10,
          right: 10,
          backgroundColor: colors.background.card,
      ...
      {onPacketCreate && (
        <div className="create-packet-panel" style={{
          position: 'absolute',
          bottom: 10,
          right: 10,
        }}>
          <button 
            onClick={() => onPacketCreate({
              header: {
                streamId,
                timestamp: Date.now(),
              }
