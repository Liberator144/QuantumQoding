...
// 1. Define explicit interfaces for orbitNodes and features
interface OrbitNodeFeature {
  name: string;
  description: string;
...
const QuantumSphere: React.FC = () => {
  ...
  const [currentView, setCurrentView] = useState<'hub' | 'star-system'>('hub')
  const [selectedStar, setSelectedStar] = useState<(typeof orbitNodes)[0] | null>(null)
  ...
  const navigateToStarSystem = (star: (typeof orbitNodes)[0]) => {
  ...
  const handleStarHover = (name: string | null) => {
    if (name && !hoveredStar) {
  ...
  return (
    <div className="relative w-full h-[100vh] overflow-hidden bg-[#050714]">
          ...
          >
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              ...
              >
                ...
                {[...Array(8)].map((_, i) => (
                  <motion.div
                    key={`ring-${selectedStar?.name ?? 'default'}-${i}`}
                    className="absolute border-2 rounded-full border-opacity-60"
                    style={{
                      width: `${(i + 1) * 40}px`,
                      height: `${(i + 1) * 40}px`,
                ...
                <div className="relative w-40 h-40">
                  ...
                  {[...Array(12)].map((_, i) => (
                    <motion.div
                      key={`flare-${selectedStar?.name ?? 'default'}-${i}`}
                      className="absolute"
                      style={{
                        width: '150%',
                        height: '2px',
              ...
              {[...Array(80)].map((_, i) => (
                <motion.div
                  key={`outerparticle-${i}-${selectedStar?.name ?? 'default'}`}
                  className="absolute rounded-full"
                  style={{
      ...
      {transitionState !== 'none' && (
        <>
          <audio
            src={`/audio/${transitionState === 'entering' ? 'wormhole-enter.mp3' : 'wormhole-exit.mp3'}`}
            autoPlay
          >
            <track kind="captions" />
          </audio>
          <audio
            src={`/audio/${transitionState === 'entering' ? 'quantum-surge.mp3' : 'quantum-collapse.mp3'}`}
            autoPlay
          >
            <track kind="captions" />
          </audio>
      ...
      {currentView === 'hub' && (
        ...
        >
          ...
          {['inner', 'middle', 'outer'].map((orbit, i) => (
            <div
              key={orbit}
              className="absolute -translate-x-1/2 -translate-y-1/2 left-1/2 top-1/2"
            >
              <div
                className="rounded-full border border-[#1a2040]/30 relative"
                style={{
                  width: `${(i + 1) * 400}px`,
                  height: `${(i + 1) * 400}px`,
                }}
                ...
                <div className="absolute inset-0 bg-gradient-radial from-[#2d1b4e] via-[#6e3bc4] to-[#b83280] animate-pulse">
                  ...
                  {[...Array(8)].map((_, i) => {
                    ...
                    return (
                      <motion.div
                        key={streamKey}
                        className="absolute top-1/2 left-1/2 h-full w-[1px] origin-center"
                        style={{
                          background:
                            'linear-gradient(180deg, transparent, rgba(56, 189, 248, 0.8), transparent)',
          ...
          {orbitNodes.map((node) => (
            <Star
              key={`star-${node.name}`}
              node={node}
              isHovered={hoveredStar === node.name}
      ...
      {/* Star System View */}
      {currentView === 'star-system' && selectedStar && (
        <StarSystemView 
          star={selectedStar} 
      ...
      <div className="absolute flex gap-2 bottom-4 right-4">
        <button
          ...
          className="p-2 text-white rounded-full bg-white/10 hover:bg-white/20"
        >
          +
        </button>
        <button
