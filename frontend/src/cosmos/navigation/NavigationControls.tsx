...
interface NavigationControlsProps {
  ...
  /** Whether to show level selector */
  showLevelSelector?: boolean;
  
  /** Whether to show history */
  showHistory?: boolean;
  
  /** Whether to show path */
  showPath?: boolean;
...
const NavigationControls: React.FC<NavigationControlsProps> = ({
   ...
   * Get level name
   */
  const getLevelName = (level: VisualizationLevel): string => {
    switch (level) {
   ...
   * Get level color
   */
  const getLevelColor = (level: VisualizationLevel): string => {
    switch (level) {
      case VisualizationLevel.UNIVERSE:
  ...
  const renderBackButton = () => {
    ...
    return (
      <button
        className={`p-2 rounded-full ${getThemeClasses()} hover:bg-opacity-70 transition-colors`}
  ...
  const renderCurrentLocation = () => {
    return (
      <div
        className={`px-3 py-1.5 rounded-lg ${getThemeClasses()} flex items-center space-x-2`}
        onClick={() => setShowLevelPanel(!showLevelPanel)}
      >
        {/* Level indicator */}
        <div
          className="w-3 h-3 rounded-full"
          style={{ backgroundColor: currentEntityColor || getLevelColor(currentLevel) }}
        />
        ...
        {showLevelSelector && (
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className={`h-4 w-4 transition-transform ${showLevelPanel ? 'rotate-180' : ''}`}
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path
  ...
  const renderLevelSelectorPanel = () => {
    if (!showLevelSelector || !showLevelPanel) return null;
    ...
    return (
      <UITransition
        ...
        } left-0 p-2 rounded-lg ${getThemeClasses()} min-w-[200px]`}
      >
        ...
        <div className="space-y-1">
          {levels.map((level) => (
            <div
              key={`level-${level}`}
              className={`flex items-center text-xs cursor-pointer hover:bg-white hover:bg-opacity-10 rounded px-2 py-1 transition-colors ${
                currentLevel === level ? 'bg-white bg-opacity-10' : ''
              }`}
              onClick={() => {
                // Close panel
  ...
  const renderHistoryPanel = () => {
    if (!showHistory || !showHistoryPanel) return null;
    ...
    return (
      <UITransition
        ...
        className={`absolute ${
          position.includes('bottom') ? 'bottom-16' : 'top-16'
        } right-0 p-2 rounded-lg ${getThemeClasses()} min-w-[200px] max-w-[300px]`}
      >
        ...
        <div className="space-y-1 max-h-40 overflow-y-auto">
          {navigationHistory.map((item, index) => (
            <div
              ...
              className={`flex items-center text-xs cursor-pointer hover:bg-white hover:bg-opacity-10 rounded px-2 py-1 transition-colors ${
                index === navigationHistory.length - 1 ? 'bg-white bg-opacity-10' : ''
              }`}
              onClick={() => {
                // Close panel
    ...
    return (
      <div className={`absolute top-4 left-1/2 transform -translate-x-1/2 z-50`}>
        <UITransition
          ...
          preset="glide"
          direction="down"
          className={`px-4 py-2 rounded-lg ${getThemeClasses()}`}
        >
