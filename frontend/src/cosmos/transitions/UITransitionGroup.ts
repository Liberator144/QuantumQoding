...
const UITransitionGroup = ({ show, staggerAmount = 0.05, staggerDirection = 'forward', totalDuration, wrapperPreset, wrapperDirection = 'right', className = '', itemClassName = '', itemProps = {}, onAnimationComplete, onAnimationStart, children }) => {
    ...
    const groupContent = (_jsx("div", { className: `ui-transition-group ${className}`, children: React.Children.map(children, (child, index) => {
    ...
    if (!wrapperPreset) {
        return (_jsx(AnimatePresence, { mode: "wait", onExitComplete: onAnimationComplete, children: show && groupContent }));
    }
    // Create a wrapper animation for the entire group
    return (_jsx(AnimatePresence, { mode: "wait", onExitComplete: onAnimationComplete, children: show && (_jsx(motion.div, { className: "ui-transition-group-wrapper", initial: "hidden", animate: "visible", exit: "exit", variants: wrapperVariants, style: { transformOrigin: 'center center' }, children: groupContent })) }));
};
