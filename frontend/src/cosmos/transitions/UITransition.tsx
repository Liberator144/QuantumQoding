...
export type UITransitionPreset =
  'slide' | 'fade' | 'zoom' | 'flip' | 'bounce' |
  'expand' | 'collapse' | 'pivot' | 'blur' | 'glide' |
  'quantum';

// Extended props for more customization options
interface UITransitionProps {
  show: boolean;
 ...
 */
export const UITransition: React.FC<UITransitionProps> = ({
  show,
  preset = 'fade',
  direction = 'right',
  duration = 0.5,
  ...
  const variants: Record<UITransitionPreset, Variants> = {
  ...
  const activeVariants = customVariants ||
    ...
    getVariantsByPreset(preset as AnimationPreset, direction as SlideDirection, {
      duration: duration,
      delay: totalDelay
    });
  ...
  return (
    <AnimatePresence mode="wait" onExitComplete={() => {
      ...
      {show && (
        <motion.div
          className={`ui-transition ${animationClass} ${className}`}
          initial="hidden"
          animate="visible"
          exit="exit"
...
// Enhanced group component for staggered and coordinated animations
interface UITransitionGroupProps {
  show: boolean;
  staggerAmount?: number;
  staggerDirection?: 'forward' | 'reverse' | 'from-center' | 'to-center' | 'random';
  totalDuration?: number;
  wrapperPreset?: UITransitionPreset;
  wrapperDirection?: TransitionDirection;
  className?: string;
  itemClassName?: string;
  progressThreshold?: number; // Show only after this progress value (0-1)
  physics?: {
    mass?: number;
    stiffness?: number;
    damping?: number;
...
export const UITransitionGroup: React.FC<UITransitionGroupProps> = ({
  show,
  staggerAmount = 0.05,
