...
// Enhanced keyframe type for more complex animations
type KeyframeValue = number | string | [number, number, number] | { x: number, y: number, z: number };

interface KeyframeConfig {
  values: KeyframeValue[];
  times: number[]; // 0-1 range for each keyframe
  ease?: string | Array<number>; // Support for custom cubic-bezier curves
}

// Support for layered camera effects
interface CameraLayer {
  type: 'transform' | 'filter' | 'color' | 'overlay';
  keyframes: Record<string, KeyframeConfig>;
  options?: Record<string, any>;
}

export type CameraTransitionMode = 'zoom' | 'pan' | 'rotate' | 'swirl' | 'dive' | 'rise' | 'tilt' | 'quantum' | 'custom';

// Milestone events for UI coordination
export type TransitionMilestone = 'start' | 'quarter' | 'third' | 'half' | 'two-thirds' | 'three-quarters' | 'almost-complete' | 'complete';
...
interface CameraEffectsProps {
  ...
  zoomFactor?: number;
  rotationDegrees?: {x?: number, y?: number, z?: number} | number;
  blurAmount?: number;
  ...
  useReverseRotation?: boolean;
  easeType?: string | Array<number>;
  layers?: CameraLayer[];
  intensity?: number; // 0-1 scale for effect intensity
  shouldShake?: boolean; // Camera shake effect
  backfaceVisible?: boolean; // Control backface visibility
  zIndexDuringTransition?: number; // Control z-index during transition
 ...
 */
const CameraEffects: React.FC<CameraEffectsProps> = ({
  isTransitioning,
  transitionState,
  mode = 'zoom',
  zoomFactor = 1.2,
  rotationDegrees = { x: 5, y: -3, z: 2 },
  blurAmount = 4,
  vignetteAmount = 30,
  perspective = 1000,
  translate3d = { x: 25, y: -15, z: 50 },
  ...
  const lastMilestoneRef = useRef<TransitionMilestone | null>(null);
  ...
  const checkMilestones = useCallback((currentProgress: number) => {
    if (!onTransitionMilestone) return;
    
    const milestones: Array<[TransitionMilestone, number]> = [
      ['quarter', 0.25],
      ['third', 0.33],
      ['half', 0.5],
      ['two-thirds', 0.66],
  ...
  const getVignetteIntensity = (baseAmount: number, p: number): number => {
  ...
  const getCameraVariants = (): Variants => {
  ...
  const getKeyframedValue = (start: number, end: number, progress: number): number => {
    if (useAcceleration) {
  ...
  return (
    <div 
      className={`w-full h-full transition-transform ${vignetteStyle} preserve-3d perspective-${perspective}`}
      style={perspectiveStyle}
    >
      <motion.div 
        className={`w-full h-full transform-gpu ${blurStyle} ${shakeClass} ${backfaceClass} preserve-3d`}
        animate={controls}
        initial="initial"
        variants={getCameraVariants()}
      >
        {children}
      </motion.div>
    </div>
