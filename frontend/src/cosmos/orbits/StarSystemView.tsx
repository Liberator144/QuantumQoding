...
export const StarSystemView: React.FC<StarSystemProps> = ({ star, onReturnToHub }) => {
  ...
  const getOrbits = () => {
    const orbits: {
      inner: Array<typeof star.features[0]>;
      middle: Array<typeof star.features[0]>;
      outer: Array<typeof star.features[0]>;
    } = {
      inner: [],
      middle: [],
      outer: []
  ...
  const distributeFeatures = (features: Array<typeof star.features[0]>, orbitRadius: number) => {
  ...
  const deriveColorFromStar = (starColor: string, index: number, totalPlanets: number) => {
  ...
  return (
    <div className="relative w-full h-full bg-[#050714] overflow-hidden">
      {/* Particle Background */}
      <div className="absolute inset-0">
        <SparklesCore
          id="star-system-sparkles"
          background="transparent"
          particleColor={star.color}
          particleDensity={80}
          speed={0.8}
          minSize={0.5}
          maxSize={1}
      ...
      <motion.button
        className="absolute top-4 left-4 z-50 flex items-center space-x-2 px-4 py-2 rounded-full"
        style={{
      ...
      >
        ...
        <p 
          className="text-sm text-center mt-1 max-w-md"
          style={{ color: `${star.color}CC` }}
        >
          {star.description}
        </p>
      </motion.div>
      ...
      <AnimatePresence>
        {showFeatureDetails && selectedFeature && (
          ...
          >
            ...
            {/* Modal content */}
            <motion.div
              className="relative z-10 bg-[#0d0d1f] max-w-lg w-full rounded-lg overflow-hidden shadow-2xl"
              style={{ 
                borderWidth: '1px',
                borderColor: `${star.color}40`,
      ...
      <div className="absolute inset-0 z-15 pointer-events-none">
        <svg className="w-full h-full">
          {hoveredPlanet && allPlanets.filter(p => p.name === hoveredPlanet).map(planet => (
            allPlanets
              .filter(p => p.name !== planet.name && p.orbit === planet.orbit)
              .map((relatedPlanet, i) => (
                <g key={`connection-${planet.name}-${relatedPlanet.name}`}>
                  <motion.line
                    x1={`calc(50% + ${Math.cos((planet.angle * Math.PI) / 180) * planet.distance}px)`}
                    y1={`calc(50% + ${Math.sin((planet.angle * Math.PI) / 180) * planet.distance}px)`}
                    x2={`calc(50% + ${Math.cos((relatedPlanet.angle * Math.PI) / 180) * relatedPlanet.distance}px)`}
                    y2={`calc(50% + ${Math.sin((relatedPlanet.angle * Math.PI) / 180) * relatedPlanet.distance}px)`}
                    stroke={planet.color}
                    strokeWidth="1"
                    strokeDasharray="3,3"
                    initial={{ opacity: 0 }}
