...
// Props interface
interface StarSystemScreenProps {
  starSystem?: string;
}
 ...
 * Star system screen component
 */
const StarSystemScreen: React.FC<StarSystemScreenProps> = ({ starSystem }) => {
  ...
  const handleFeatureSelect = (featurePath: string) => {
    if (!starSystemData) return;
    ...
    <StarSystemRoute>
      <div className="flex flex-col w-full h-full p-6">
        ...
        <div className="grid flex-1 grid-cols-3 gap-6">
          {starSystemData.features.map(feature => (
            <motion.div
              key={feature.path}
              className="relative overflow-hidden rounded-lg shadow-lg cursor-pointer"
              style={{ backgroundColor: `${starSystemData.color}15` }}
              whileHover={{ 
                scale: 1.03,
            ...
            >
              ...
              <div 
                ...
                style={{ backgroundColor: starSystemData.color }}
              />
              
              <div 
                className="absolute top-0 left-0 w-4 h-4 rounded-br-full opacity-30"
                style={{ backgroundColor: starSystemData.color }}
              />
            </motion.div>
          ))}
