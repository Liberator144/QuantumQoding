...
const HubScreen = () => {
    ...
    return (<div className="relative flex items-center justify-center w-full h-full overflow-hidden bg-black">
      {/* Background stars */}
      <div className="absolute inset-0 pointer-events-none">
        {Array.from({ length: 100 }, (_, i) => (<motion.div key={`star-${i}`} className="absolute rounded-full bg-white" style={{
      ...
      <div className="relative z-10 w-full h-full">
        <QuantumSphere starSystems={starSystems} onStarSystemSelect={handleStarSystemSelect}/>
      </div>

      {/* Hub title */}
      <motion.div className="absolute top-0 left-0 right-0 z-20 p-6 text-center" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.5 }}>
