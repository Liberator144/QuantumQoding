...
export class SupabaseAdapter {
    ...
    constructor(config) {
        /** Adapter name */
        this.name = 'SupabaseAdapter';
        /** Adapter data source type */
        this.dataSourceType = DataSourceType.SUPABASE;
        /** Entity cache */
    ...
    async fetchDatabaseInfo() {
        try {
            ...
            for (const table of tables) {
                ...
                const dbTableRelationship = this.createRelationship(RelationshipType.PARENT_CHILD, databaseEntity.id, tableEntity.id, 'has table', 'Database has table');
                ...
                for (const column of columns) {
                    ...
                    const tableFieldRelationship = this.createRelationship(RelationshipType.PARENT_CHILD, tableEntity.id, fieldEntity.id, 'has field', 'Table has field');
                ...
                for (const record of records) {
                    ...
                    const tableRecordRelationship = this.createRelationship(RelationshipType.PARENT_CHILD, tableEntity.id, recordEntity.id, 'has record', 'Table has record');
            ...
            for (const foreignKey of foreignKeys) {
                ...
                const tableRelationship = this.createRelationship(RelationshipType.REFERENCE, sourceTableId, targetTableId, foreignKey.constraint_name, `${foreignKey.source_table}.${foreignKey.source_column} references ${foreignKey.target_table}.${foreignKey.target_column}`);
     ...
     */
    createFieldEntity(data, tableId) {
        return {
     ...
     */
    createRecordEntity(data, tableName, tableId) {
