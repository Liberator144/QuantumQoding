...
import { EntityType, RelationshipType, DataSourceType, } from '../types';
/**
...
export class GitHubAdapter {
    ...
    constructor(config = {}) {
        /** Adapter name */
        this.name = 'GitHubAdapter';
        /** Adapter data source type */
        this.dataSourceType = DataSourceType.GITHUB;
        /** Entity cache */
    ...
    async getEntities(query) {
        ...
        if (query) {
            ...
            if (query.name) {
                ...
                entities = entities.filter(entity => entity.name.toLowerCase().includes(nameLower));
            }
            if (query.owner) {
                const ownerLower = query.owner.toLowerCase();
                entities = entities.filter(entity => entity.owner?.toLowerCase().includes(ownerLower));
            }
            if (query.parentId) {
                entities = entities.filter(entity => entity.parentId === query.parentId);
            }
            if (query.properties) {
    ...
    async getRelationships(query) {
        ...
        if (query) {
            if (query.type) {
                relationships = relationships.filter(relationship => relationship.type === query.type);
            }
            if (query.sourceId) {
                relationships = relationships.filter(relationship => relationship.sourceId === query.sourceId);
            }
            if (query.targetId) {
                relationships = relationships.filter(relationship => relationship.targetId === query.targetId);
            }
            // Apply pagination
            if (query.offset !== undefined || query.limit !== undefined) {
                const offset = query.offset || 0;
     ...
     */
    async fetchOrganizationData(organization) {
        // Fetch organization
        ...
        for (const repoData of reposData) {
            ...
            const relationship = this.createRelationship(RelationshipType.OWNERSHIP, orgEntity.id, repoEntity.id, 'owns', 'Organization owns repository');
        ...
        for (const memberData of membersData) {
            ...
            const relationship = this.createRelationship(RelationshipType.MEMBERSHIP, orgEntity.id, memberEntity.id, 'has member', 'Organization has member');
            this.relationshipCache.set(relationship.id, relationship);
        }
    }
    /**
    ...
    async fetchRepositoryData(owner, repo) {
        ...
        for (const issueData of issuesData) {
            ...
            const repoIssueRelationship = this.createRelationship(RelationshipType.PARENT_CHILD, repoEntity.id, issueEntity.id, 'has issue', 'Repository has issue');
            ...
            if (issueData.user) {
                ...
                const userIssueRelationship = this.createRelationship(RelationshipType.CREATION, userEntity.id, issueEntity.id, 'created', 'User created issue');
            ...
            if (issueData.assignee) {
                ...
                const assigneeIssueRelationship = this.createRelationship(RelationshipType.ASSIGNMENT, assigneeEntity.id, issueEntity.id, 'assigned to', 'Issue assigned to user');
        ...
        for (const prData of prsData) {
            ...
            const repoPrRelationship = this.createRelationship(RelationshipType.PARENT_CHILD, repoEntity.id, prEntity.id, 'has pull request', 'Repository has pull request');
            ...
            if (prData.user) {
                ...
                this.entityCache.set(userEntity.id, userEntity);
                const userPrRelationship = this.createRelationship(RelationshipType.CREATION, userEntity.id, prEntity.id, 'created', 'User created pull request');
        ...
        for (const branchData of branchesData) {
            ...
            const repoBranchRelationship = this.createRelationship(RelationshipType.PARENT_CHILD, repoEntity.id, branchEntity.id, 'has branch', 'Repository has branch');
        ...
        for (const contributorData of contributorsData) {
            ...
            const repoContributorRelationship = this.createRelationship(RelationshipType.ASSOCIATION, repoEntity.id, contributorEntity.id, 'has contributor', 'Repository has contributor');
     ...
     */
    createBranchEntity(data, repoId) {
        return {
     ...
     * @returns Relationship
     */
    createRelationship(type, sourceId, targetId, name, description) {
        return {
            id: uuidv4(),
            type,
            sourceId,
