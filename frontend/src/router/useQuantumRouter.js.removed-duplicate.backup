...
export const useQuantumRouter = () => {
     ...
     */
    const quantumNavigate = useCallback((to, options = {}) => {
        ...
        const targetStarSystem = starSystems.find(system => to.startsWith(system.path));
     ...
     */
    const navigateToStarSystem = useCallback((starSystemName, options = {}) => {
        // Find star system
        const starSystem = starSystems.find(system => system.name === starSystemName);
        if (!starSystem) {
     ...
     */
    const navigateToFeature = useCallback((starSystemName, featurePath, options = {}) => {
        // Find star system
        const starSystem = starSystems.find(system => system.name === starSystemName);
        if (!starSystem) {
            console.error(`Star system "${starSystemName}" not found`);
