...
export const StarSystemRoute = ({ children }) => {
    ...
    return (<motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.5 }} className="relative w-full h-full overflow-hidden rounded-lg" style={starSystemStyles}>
      {/* Star system background effect */}
      {currentStarSystem && (<div className="absolute inset-0 pointer-events-none">
          ...
          {/* Orbital lines */}
          {[...Array(3)].map((_, i) => (<div key={`orbital-${i}`} className="absolute rounded-full border opacity-10" style={{
                    width: `${(i + 1) * 50}%`,
                    height: `${(i + 1) * 50}%`,
                    top: `${50 - (i + 1) * 25}%`,
                    left: `${50 - (i + 1) * 25}%`,
