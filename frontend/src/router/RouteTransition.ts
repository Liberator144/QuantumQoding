...
export const RouteTransition = ({ children, transitionType = 'fade', duration = 0.5, }) => {
    ...
    return (_jsx(AnimatePresence, { mode: "wait", onExitComplete: handleTransitionStart, children: _jsx(motion.div, { initial: variants[transitionType].initial, animate: variants[transitionType].animate, exit: variants[transitionType].exit, transition: {
                duration,
                ease: transitionType === 'wormhole' ? 'easeInOut' : 'easeOut'
            }, className: "w-full h-full", children: children }, location.pathname) }));
};
