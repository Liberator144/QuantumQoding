 ...
 * Migrated from: QuantumButton.js
 * @version 2.0.0
 */
/**
 * Quantum Button Component
 *
 * This component provides a quantum-themed button with various styles and animations.
 *
 * @version 1.0.0
 */
...
const QuantumButton = ({ children, variant = 'primary', size = 'md', disabled = false, loading = false, fullWidth = false, onClick, className = '', type = 'button', icon, iconPosition = 'left', quantumEffect = true, }) => {
    ...
    return (<motion.button type={type} disabled={disabled || loading} onClick={handleClick} className={`
        ...
        ${className}
      `} whileTap={{ scale: disabled || loading ? 1 : 0.98 }} whileHover={quantumEffect && !disabled && !loading ? 'hover' : undefined} variants={quantumVariants}>
