...
class AdaptiveSecurityManager extends EventEmitter {
  ...
  _setupEventListeners() {
    ...
    securityManager.on('user-authenticated', data =>
      this._handleAuthenticationEvent(data, 'success')
    );
    securityManager.on('authentication-failed', data =>
      this._handleAuthenticationEvent(data, 'failure')
    );

    // Authorization events
    securityManager.on('authorization-failed', data =>
      this._handleAuthorizationEvent(data, 'failure')
    );
  ...
  async trainAnomalyDetectionModel() {
    try {
      ...
      for (const userId in userEvents) {
        ...
        if (userEventsList.length < 5) continue;
