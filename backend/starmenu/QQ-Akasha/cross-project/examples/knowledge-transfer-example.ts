...
async function runExample() {
  ...
  const codePatternA = await system.transferManager.createKnowledge(
    KnowledgeType.CODE_PATTERN,
    'API Request Handler',
    'A reusable pattern for handling API requests with error handling and loading states',
    `
function useApiRequest<T>(apiFunction: () => Promise<T>) {
  ...
  return { data, loading, error, execute };
    ...
    `,
    'project-a',
    ['api', 'hooks', 'react', 'error-handling'],
    {
      language: 'typescript',
      compatibility: {
        languages: ['typescript', 'javascript'],
  ...
  const bestPracticeA = await system.transferManager.createKnowledge(
...
6. Use React.memo for performance optimization when appropriate
  ...
  const codePatternB = await system.transferManager.createKnowledge(
    KnowledgeType.CODE_PATTERN,
    'Mobile Navigation',
    'A pattern for handling navigation in mobile applications',
    `
...
function AppNavigator() {
  return (
    ...
    `,
    'project-b',
    ['navigation', 'mobile', 'react-native'],
    {
      language: 'typescript',
      compatibility: {
        languages: ['typescript', 'javascript'],
  ...
  console.log(
    `Created knowledge entities: ${[codePatternA, bestPracticeA, codePatternB].map(k => k.title).join(', ')}`
  );
  ...
  for (const result of searchResults) {
    console.log(
      `- "${result.knowledge.title}" (Relevance: ${result.relevance.toFixed(2)}, Compatibility: ${result.compatibilityScore.toFixed(2)})`
    );
  ...
  const graphA = await system.graphBuilder.buildGraph(
    { sourceProject: 'project-a' },
    { name: 'Project A Knowledge Graph' }
  );

  // Build graph for Project B
  const graphB = await system.graphBuilder.buildGraph(
    { sourceProject: 'project-b' },
    { name: 'Project B Knowledge Graph' }
  );

  console.log(
    `Built graphs: ${graphA.name} (${graphA.nodes.size} nodes), ${graphB.name} (${graphB.nodes.size} nodes)`
  );
  ...
  console.log(
    `Merged graph: ${mergeResult.graph.name} (${mergeResult.graph.nodes.size} nodes, ${mergeResult.graph.relationships.size} relationships)`
  );
  console.log(
    `Created ${mergeResult.stats.newCrossGraphRelationships} cross-project relationships`
  );
  ...
  if (searchResults.length > 0) {
    ...
    const translationResult = await system.translator.translateKnowledge(
      knowledgeToApply,
      system.projectManager.getProject('project-b')!
    );

    console.log(
      `Translated "${knowledgeToApply.title}" with confidence ${translationResult.confidence.toFixed(2)}`
    );
    console.log(`Translation notes: ${translationResult.notes.join(' ')}`);

    // Apply the knowledge
    const application = await system.transferManager.applyKnowledge(
      knowledgeToApply.id,
      'project-b',
      {
