 ...
 */
export enum ProjectContextEvent {
  PROJECT_ADDED = 'project-added',
  PROJECT_UPDATED = 'project-updated',
  PROJECT_REMOVED = 'project-removed',
  ACTIVE_PROJECT_CHANGED = 'active-project-changed',
}
...
export class ProjectContextManager {
   ...
   */
  findProjectsByTag(tag: string): ProjectInfo[] {
   ...
   */
  findProjectsByLanguage(language: string): ProjectInfo[] {
   ...
   */
  findProjectsByFramework(framework: string): ProjectInfo[] {
    return Array.from(this.projects.values()).filter(project =>
   ...
   */
  findProjectsByDependency(dependency: string): ProjectInfo[] {
    return Array.from(this.projects.values()).filter(project =>
      Object.keys(project.dependencies).includes(dependency)
    );
  }
  ...
  findSimilarProjects(
    ...
    options: {
      ...
      considerFrameworks?: boolean;
      considerDependencies?: boolean;
      considerTags?: boolean;
      minSimilarityScore?: number;
    } = {}
  ): Array<{ project: ProjectInfo; similarityScore: number }> {
    const {
      considerLanguages = true,
      considerFrameworks = true,
    ...
    const results: Array<{ project: ProjectInfo; similarityScore: number }> = [];
    ...
    for (const targetProject of this.projects.values()) {
      ...
      if (considerLanguages && sourceProject.languages.length > 0) {
        totalFactors++;
        const commonLanguages = sourceProject.languages.filter(lang =>
          targetProject.languages.includes(lang)
        );
      ...
      if (considerFrameworks && sourceProject.frameworks.length > 0) {
        totalFactors++;
        const commonFrameworks = sourceProject.frameworks.filter(framework =>
   ...
   */
  on(event: ProjectContextEvent, listener: (data: any) => void): void {
   ...
   */
  off(event: ProjectContextEvent, listener: (data: any) => void): void {
