...
import { CodeTemplate, TemplateRepository } from '../code-generation/templates/types';
...
export class CodeGenerationStrategy extends BaseRecommendationStrategy {
  ...
  calculateRelevanceScore(
    ...
    if (
      ...
      const matchingFrameworks = analysis.frameworks.filter(framework =>
        knowledge.compatibility!.frameworks!.includes(framework)
      );
    ...
    if (analysis.patterns && analysis.patterns.length > 0) {
      ...
      const matchingPatterns = analysis.patterns.filter(pattern =>
        knowledge.tags.includes(pattern)
      );
    ...
    if (analysis.keywords.length > 0) {
      ...
      const matchingKeywords = analysis.keywords.filter(
        keyword =>
   ...
   */
  async generateApplicationMethod(
    knowledge: Knowledge,
    context: DeveloperContext
  ): Promise<KnowledgeRecommendation['applicationMethod'] | undefined> {
    ...
    try {
      ...
      if (useMultiFileContext) {
        ...
        extractionResult = await extractMultiFileContext(
          context.filePath,
          context.fileContent ?? '',
        ...
        extractionResult = await extractContext(
          context.filePath,
          context.fileContent ?? '',
          context.cursorPosition,
          context.selection
      ...
      // Select templates
      const selectionResult = selectTemplates(
        this.templateRepository,
        useMultiFileContext
          ? extractionResult.context.fileContexts.get(context.filePath)
          : extractionResult,
        {
          language: knowledge.language as any,
          tags: knowledge.tags,
        }
      );
