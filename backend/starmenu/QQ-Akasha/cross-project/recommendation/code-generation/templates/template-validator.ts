...
import { CodeTemplate, TemplateValidationResult, TemplateVariable, VariableType } from './types';

/**
 * Validate a template
 */
export function validateTemplate(template: CodeTemplate): TemplateValidationResult {
  const errors: string[] = [];
 ...
 */
function validateVariable(variable: TemplateVariable): string[] {
  const errors: string[] = [];
  ...
  if (!variable.name) {
    ...
    errors.push(
      `Variable name ${variable.name} must contain only letters, numbers, and underscores`
    );
  }

  if (!variable.type) {
    errors.push(`Variable ${variable.name} type is required`);
  } else if (!Object.values(VariableType).includes(variable.type as any)) {
