 ...
 */
function extractJavaScriptImports(content: string): ImportDeclaration[] {
 ...
 */
function extractPythonImports(content: string): ImportDeclaration[] {
  const imports: ImportDeclaration[] = [];
  ...
  const importRegex =
    /^(?:import\s+([^,\s]+)(?:\s+as\s+([^,\s]+))?|from\s+([^,\s]+)\s+import\s+(.+))$/gm;
 ...
 */
function extractJavaImports(content: string): ImportDeclaration[] {
  const imports: ImportDeclaration[] = [];
 ...
 */
function extractCSharpImports(content: string): ImportDeclaration[] {
  const imports: ImportDeclaration[] = [];
 ...
 */
function extractGoImports(content: string): ImportDeclaration[] {
  const imports: ImportDeclaration[] = [];
 ...
 */
function extractRustImports(content: string): ImportDeclaration[] {
  const imports: ImportDeclaration[] = [];
  ...
  for (const line of lines) {
    ...
    while ((match = useRegex.exec(line)) !== null) {
      ...
      if (importPath.includes('{')) {
        ...
        const groupContent = importPath.substring(
