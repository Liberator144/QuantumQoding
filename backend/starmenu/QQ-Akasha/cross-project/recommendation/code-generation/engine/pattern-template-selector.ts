...
import { DesignPatternType, PatternDetectionResult } from '../patterns/types';
...
/**
 * Pattern-based template selection options
 */
export interface PatternTemplateSelectionOptions extends TemplateSelectionOptions {
  /** Whether to prioritize pattern-based templates */
  prioritizePatternTemplates?: boolean;
 ...
 */
export const DEFAULT_PATTERN_TEMPLATE_SELECTION_OPTIONS: PatternTemplateSelectionOptions = {
  ...DEFAULT_TEMPLATE_SELECTION_OPTIONS,
 ...
 */
export function selectTemplatesBasedOnPatterns(
  repository: TemplateRepository,
  fileContext: FileContext,
  options: Partial<PatternTemplateSelectionOptions> = {}
): TemplateSelectionResult {
  // Merge options with defaults
  const mergedOptions: PatternTemplateSelectionOptions = {
  ...
  const filteredPatterns = detectedPatterns.filter(
    pattern => pattern.confidence >= mergedOptions.minPatternConfidence!
  );
 ...
 */
function buildQueryOptionsFromPatterns(
  ...
  if (
    patterns.some(
      p =>
        p.pattern === DesignPatternType.SINGLETON ||
    ...
    patterns.some(
      p =>
        p.pattern === DesignPatternType.ADAPTER ||
    ...
    patterns.some(
      p =>
        p.pattern === DesignPatternType.CHAIN_OF_RESPONSIBILITY ||
 ...
 */
function calculatePatternTemplateRelevance(
  ...
  if (
    template.category === TemplateCategory.CREATIONAL &&
    patterns.some(
      p =>
        p.pattern === DesignPatternType.SINGLETON ||
        p.pattern === DesignPatternType.FACTORY ||
    ...
    template.category === TemplateCategory.STRUCTURAL &&
    patterns.some(
      p =>
        p.pattern === DesignPatternType.ADAPTER ||
        p.pattern === DesignPatternType.BRIDGE ||
    ...
    template.category === TemplateCategory.BEHAVIORAL &&
    patterns.some(
      p =>
        p.pattern === DesignPatternType.CHAIN_OF_RESPONSIBILITY ||
        p.pattern === DesignPatternType.COMMAND ||
