...
import { CodeTemplate, TemplateVariable, VariableType } from '../templates/types';
import { VariableValue } from '../templates/variable-substitution';
...
function extractIdentifier(
  ...
  if (fileContext.currentScope) {
    ...
    const nameWords = nameParts.filter(
      part => part.length > 2 && !['the', 'and', 'for', 'with', 'that', 'this'].includes(part)
    );
...
function extractMember(
  ...
  const namingConvention =
    variable.type === VariableType.METHOD
...
function extractCustom(
  ...
  if (variable.name.includes('functionName') || variable.name.includes('function_name')) {
    ...
    const functionElement = fileContext.elements.find(
