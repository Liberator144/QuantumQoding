...
import { InteractionType, UserInteraction, UserPreference } from '../types';

/**
 * Interaction analytics result
 */
...
export interface PreferenceAnalytics {
  ...
  preferencesBySource: Record<string, number>;
...
export class AnalyticsManager {
   ...
   */
  analyzeInteractions(interactions: UserInteraction[]): InteractionAnalytics {
   ...
   */
  analyzePreferences(preferences: UserPreference[]): PreferenceAnalytics {
    // Count preferences by category
    const preferencesByCategory: Record<string, number> = {};
    ...
    const preferencesBySource: Record<string, number> = {};
    for (const preference of preferences) {
    ...
    const preferenceValueCounts: Record<string, number> = {};
    for (const preference of preferences) {
   ...
   */
  private categorizeActivityLevel(totalInteractions: number): string {
   ...
   */
  private getMostFrequentActivity(
    interactionsByType: Record<string, number>
   ...
   */
  private analyzeTimePattern(activeHours: number[]): string {
   ...
   */
  private analyzeContextSwitching(
   ...
   */
  private categorizePreferenceStrength(averageConfidence: number): string {
   ...
   */
  private getDominantCategories(preferencesByCategory: Record<string, number>): string[] {
   ...
   */
  private analyzePreferenceConsistency(preferences: UserPreference[]): string {
   ...
   */
  private analyzeWorkflowStyle(interactions: UserInteraction[]): string {
    // This is a simplified implementation
    const queryCount = interactions.filter(i => i.type === InteractionType.QUERY).length;
    const codeGenCount = interactions.filter(
      i => i.type === InteractionType.CODE_GENERATION
    ).length;
    const docGenCount = interactions.filter(
      i => i.type === InteractionType.DOCUMENTATION_GENERATION
    ).length;
   ...
   */
  private analyzeFeedbackPattern(interactions: UserInteraction[]): string {
   ...
   */
  private analyzeLearningCurve(interactions: UserInteraction[]): string {
  ...
  private generateRecommendations(
    ...
    if (insights.behavioralPatterns.workflowStyle === 'code-focused') {
      ...
      recommendations.push(
        'Consider enabling advanced documentation style personalization features'
      );
    }
    ...
    if (insights.behavioralPatterns.feedbackPattern === 'mostly negative') {
      recommendations.push(
        'Review and adjust your current preferences to better match your expectations'
      );
    }
