...
import { MemoryRelationshipType } from './types';
export class GraphBuilder {
    ...
    async buildGraph(query = {}, options = {}) {
        const { includeRelated = true, maxRelatedDepth = 1, title = 'Memory Graph', description = 'Generated memory visualization', } = options;
        // Get initial memories from query
        const result = await this.memoryBank.queryMemories(query);
        const memories = result.memories;
        // Create nodes for each memory
        const nodes = memories.map(memory => this.createNodeFromMemory(memory));
        ...
        for (const memory of memories) {
            if (includeRelated && memory.relatedMemories && memory.relatedMemories.length > 0) {
                await this.addRelatedMemories(memory, nodeMap, edges, processedRelationships, maxRelatedDepth, 0);
            }
        }
     ...
     */
    async addRelatedMemories(memory, nodeMap, edges, processedRelationships, maxDepth, currentDepth) {
        ...
        for (const relatedId of memory.relatedMemories) {
            ...
            if (!nodeMap.has(relatedId)) {
                ...
                if (currentDepth + 1 < maxDepth) {
                    await this.addRelatedMemories(relatedMemory, nodeMap, edges, processedRelationships, maxDepth, currentDepth + 1);
                }
            }
            // Add edge
     ...
     */
    createEdgesBetweenNodes(nodes, edges, processedRelationships) {
        ...
        for (const [tag, tagNodes] of tagMap.entries()) {
            ...
            for (let i = 0; i < tagNodes.length; i++) {
                for (let j = i + 1; j < tagNodes.length; j++) {
                    ...
                    if (strength >= 0.2) {
                        const edge = this.createEdge(sourceId, targetId, MemoryRelationshipType.SIMILAR_TO, strength);
                        edges.push(edge);
                    }
                }
            }
     ...
     */
    createEdge(sourceId, targetId, type, strength = 0.5, direction = 'bi') {
        return {
            id: uuidv4(),
            source: sourceId,
            target: targetId,
