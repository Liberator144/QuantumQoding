...
async function runExample() {
  ...
  return a + b;
    ...
    const docMemory = await akasha.createMemory(
      'The sum function adds two numbers and returns the result.',
      MemoryType.DOCUMENTATION,
      ['function', 'documentation', 'math'],
      {},
    ...
    const decisionMemory = await akasha.createMemory(
      'We decided to implement the math utilities as pure functions for better testability.',
      MemoryType.DECISION,
    ...
    highPriorityMemories.forEach(memory => {
      console.log(
        `- ${memory.id} (priority: ${memory.priorityScore?.toFixed(2)}): ${memory.content.substring(0, 50)}...`
      );
    });
    ...
    fs.writeFileSync(
      path.join(outputDir, 'memory-graph.json'),
