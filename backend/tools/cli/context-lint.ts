  ...
  if (options.verbose) {
    ...
    if (result.type === 'directory' && result.fileContexts) {
      ...
      for (const fileContext of result.fileContexts) {
        console.log(
          `  ${path.relative(process.cwd(), fileContext.path)}: ${fileContext.context} (${(fileContext.confidence * 100).toFixed(2)}%)`
        );
      }
    }
  }
...
function displayLintResult(result, options) {
  ...
  if (result.result.fileCount) {
    ...
    console.log(
      `Found ${chalk.red(`${errorCount} errors`)} and ${chalk.yellow(`${warningCount} warnings`)}`
    );
    ...
    if (fileResult.eslint) {
      if (fileResult.eslint.error) {
        ...
        for (const message of fileResult.eslint.messages) {
          ...
          console.log(
            `    ${chalk[msgColor](`${message.line}:${message.column} - ${message.message} (${message.ruleId})`)}`
          );
        }
      }
    }
    ...
    if (fileResult.markdownlint) {
      if (fileResult.markdownlint.error) {
        ...
        for (const message of fileResult.markdownlint.messages) {
          console.log(
            `    ${chalk.red(`${message.lineNumber}: ${message.ruleDescription} (${message.ruleNames.join(', ')})`)}`
          );
        }
      }
    }
