...
class Collection {
  ...
  async _saveData() {
    try {
      // Save data to adapter
      const result = await this.adapter.save(this.name, this.data);
  ...
  _validateDocument(doc) {
    ...
    for (const [field, schema] of Object.entries(this.schema)) {
      if (schema.required && (doc[field] === undefined || doc[field] === null)) {
    ...
    for (const [field, value] of Object.entries(doc)) {
      ...
      if (fieldSchema.type) {
        ...
        if (actualType !== fieldSchema.type) {
          errors.push(
            `Invalid type for field ${field}: expected ${fieldSchema.type}, got ${actualType}`
          );
        }
      }
      ...
      if (fieldSchema.enum && !fieldSchema.enum.includes(value)) {
        errors.push(
          `Invalid value for field ${field}: must be one of [${fieldSchema.enum.join(', ')}]`
        );
      }
    }
