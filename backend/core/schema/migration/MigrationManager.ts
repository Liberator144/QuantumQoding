...
class MigrationManager {
  ...
  _setupMigrationCollection() {
    ...
    const migrationCollection =
      this.config.database.getCollection(this.config.migrationCollection) ||
  ...
  async getMigrations(collectionName) {
    ...
    // Find migrations
    const migrations = migrationCollection.find(
      { collectionName },
      { sort: { fromVersion: 1, toVersion: 1 } }
    );
  ...
  async migrateCollection(collection, fromVersion, toVersion, options = {}) {
    ...
    if (migrationPath.length === 0) {
      throw new Error(
        `No migration path found: ${collection.name} (${fromVersion} -> ${toVersion})`
      );
    }
    ...
    for (const migration of migrationPath) {
      ...
      const migrationFn = this._getMigrationFunction(
      ...
      if (!migrationFn) {
        throw new Error(
          `Migration function not found: ${migration.collectionName} (${migration.fromVersion} -> ${migration.toVersion})`
        );
      }
      ...
      this.log(
        `Executed migration: ${migration.collectionName} (${migration.fromVersion} -> ${migration.toVersion})`
      );
    }
