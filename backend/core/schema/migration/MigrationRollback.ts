...
class MigrationRollback {
  ...
  _setupRollbackCollection() {
    ...
    const rollbackCollection =
      this.config.database.getCollection(this.config.rollbackCollection) ||
  ...
  async getRollbacks(collectionName) {
    ...
    // Find rollbacks
    const rollbacks = rollbackCollection.find(
      { collectionName },
  ...
  async rollbackMigration(collection, fromVersion, toVersion, options = {}) {
    ...
    if (!rollbackFn) {
      throw new Error(
        `Rollback function not found: ${collection.name} (${fromVersion} -> ${toVersion})`
      );
    }
