...
class IndexAwareModel {
  ...
  _getIndexInfo(query, context = {}) {
    ...
    if (matchingIndexes.length > 0) {
      ...
      const bestIndex = matchingIndexes.reduce(
        (best, index) => {
      ...
      plan.nodes.forEach((node, index) => {
        ...
        switch (nodeType) {
          ...
          case 'join':
            ...
            const joinCost =
              this.config.baseCosts.join[joinType] || this.config.baseCosts.join.nested;
          ...
          case 'sort':
            ...
            const sortCost =
              this.config.baseCosts.sort[sortType] || this.config.baseCosts.sort.inMemory;
      ...
      plan.nodes.forEach((node, index) => {
        ...
        matchingIndexes.forEach(index => {
          ...
          if (index.selectivity !== undefined) {
            ...
            index.selectivity = Math.max(
              0,
              Math.min(1, index.selectivity * (1 - nodeError * learningRate))
            );
          }
        });
