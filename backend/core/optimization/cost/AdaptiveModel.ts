...
class AdaptiveModel {
  ...
  async estimatePlanCost(plan, statistics = {}, context = {}) {
    try {
      // Get base estimate
      const baseEstimate = await this.config.baseCostModel.estimatePlanCost(
        plan,
        statistics,
        context
      );
  ...
  async update(plan, actualMetrics, context = {}) {
    try {
      ...
      if (context.enableAnomalyDetection && Math.abs(error.totalError) > context.anomalyThreshold) {
        this.log(
          `Anomaly detected: error ${error.totalError.toFixed(2)} exceeds threshold ${context.anomalyThreshold}`
        );
      ...
      this._updateWeights(
        features,
        error.totalError,
        context.learningRate || this.config.learningRate
      );
  ...
  _extractFeatures(query, context = {}) {
    ...
    if (this.state.features.size > this.config.featureExtraction.maxFeatures) {
      ...
      const featureWeights = featureArray.map(feature =>
        Math.abs(this.state.weights.features[feature] || 0)
      );
      ...
      this.state.features = new Set(
        sortedFeatures.slice(0, this.config.featureExtraction.maxFeatures).map(item => item.feature)
      );
    }
  ...
  _calculateError(actualMetrics, estimatedMetrics) {
    ...
    const totalError =
      (actualMetrics.totalCost - estimatedMetrics.totalCost) / Math.max(1, actualMetrics.totalCost);
