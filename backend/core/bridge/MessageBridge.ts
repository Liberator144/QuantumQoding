...
class MessageBridge extends EventEmitter {
  ...
  _initialize() {
    ...
    if (this.config.contextExpiration > 0) {
      this._cleanupInterval = setInterval(
        ...
        Math.min(this.config.contextExpiration / 2, 60000)
  ...
  async preserveContext(messageId, context) {
    try {
      ...
      this.state.contextStore.set(messageId, {
        ...
        storedAt: Date.now(),
        expiresAt:
          this.config.contextExpiration > 0 ? Date.now() + this.config.contextExpiration : 0,
      });
  ...
  _trimContextStore() {
    ...
    const sortedContexts = Array.from(this.state.contextStore.entries()).sort(
