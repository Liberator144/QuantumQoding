    ...
    for (const result of results) {
      try {
        ...
        const durationChange =
          ((result.duration.avg - previousResult.duration.avg) / previousResult.duration.avg) * 100;

        // Calculate memory usage change
        const memoryUsageChange =
          ((result.memoryUsage.avg - previousResult.memoryUsage.avg) /
            previousResult.memoryUsage.avg) *
          100;

        // Determine if changes are significant
        const isSignificantDurationChange =
          Math.abs(durationChange) >= this.config.significantChangeThreshold;
        const isSignificantMemoryChange =
          Math.abs(memoryUsageChange) >= this.config.significantChangeThreshold;
    ...
    if (significantChanges > 0) {
      ...
      if (durationChanges.length > 0) {
        const avgDurationChange =
          durationChanges.reduce((sum, change) => sum + change, 0) / durationChanges.length;
