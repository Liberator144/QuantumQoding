...
class GuidelineChecker extends EventEmitter {
        ...
        items: [
          {
            ...
            key: 'componentGuides',
            description:
              'Create comprehensive guides in the `docs/guides/` directory for all new significant components or modules.',
            required: true,
            status: 'pending',
            notes: '',
          },
          {
            ...
            key: 'exampleFiles',
            description:
              'Create illustrative example files in the `examples/` directory to demonstrate practical usage of new components or features.',
            required: true,
            status: 'pending',
            notes: '',
          },
          {
            ...
            key: 'unitTests',
            description:
              'Create comprehensive unit tests for all individual components, classes, and non-trivial functions.',
            required: true,
            status: 'pending',
            notes: '',
          },
  ...
  _extractDocumentationGuidelines(content) {
    ...
    return {
      ...
      jsDocComments: {
        required: true,
        description:
          'Rigorously document all public APIs (classes, methods, properties, functions) using JSDoc comments.',
      },
    };
  }
  ...
  _extractStructureGuidelines(content) {
    ...
    return {
      directoryStructure: {
        required: true,
        description:
          'Place components in the appropriate module directory. Directory names should be lowercase and hyphenated if multiple words.',
      },
      fileOrganization: {
        required: true,
        description:
          'Create a main component file using PascalCase. Create separate files for distinct subcomponents or helper classes.',
      },
      integration: {
        required: true,
        description:
          "Update the parent module's `index.js` file to re-export the new component or its encapsulating module.",
      },
    };
  }
  ...
  _extractCodeStyleGuidelines(content) {
    ...
    return {
      namingConventions: {
        required: true,
        description:
          'Use `camelCase` for variables, function names, and object properties. Use `PascalCase` for class names.',
      },
      fileStructure: {
        required: true,
        description:
          'Start each `.js` file with a JSDoc file header comment outlining its purpose and author/version if applicable.',
      },
      errorHandling: {
        required: true,
        description:
          'Utilize `try/catch` blocks for operations that are known to be error-prone. Provide meaningful, informative error messages.',
      },
    };
  }
  ...
  _extractTestingGuidelines(content) {
    ...
    return {
      ...
      integrationTests: {
        required: true,
        description:
          'Create integration tests to verify the interactions and contracts between collaborating components or modules.',
      },
      testOrganization: {
        required: true,
        description:
          'Organize test files in a `tests` directory that mirrors the `src` directory structure.',
      },
      testAutomation: {
        required: true,
  ...
  async _runChecks(targetPath, isDirectory, options) {
    ...
    const documentationResults = await this.checkers.documentation.check(
      targetPath,
      isDirectory,
      options
    );
  ...
  _generateChecklistFromGuidelines(targetPath, guidelines) {
    ...
    const documentationItems = this._generateChecklistItemsForCategory(
      'Documentation',
      guidelines.documentation
    );
    const structureItems = this._generateChecklistItemsForCategory(
      'Structure',
      guidelines.structure
    );
    const codeStyleItems = this._generateChecklistItemsForCategory(
      'Code Style',
      guidelines.codeStyle
    );
  ...
  _isViolationOverridden(violation) {
    ...
    return this.overrides.overrides.some(
      override => override.guideline === violation.guideline && override.path === violation.path
    );
  }
