  ...
  async _checkJSDocCommentsInFile(filePath) {
    try {
      ...
      for (const className of classes) {
        const hasJSDoc = jsDocComments.some(
          comment =>
            comment[0].includes(`* ${className}`) || comment[0].includes(`*\n * ${className}`)
        );
      ...
      for (const methodName of publicMethods) {
        const hasJSDoc = jsDocComments.some(
          comment =>
            comment[0].includes(`* ${methodName}`) || comment[0].includes(`*\n * ${methodName}`)
        );
      ...
      for (const funcName of publicFunctions) {
        const hasJSDoc = jsDocComments.some(
          comment =>
            comment[0].includes(`* ${funcName}`) || comment[0].includes(`*\n * ${funcName}`)
        );
