...
| **Context7 → Desktop Commander** | Documentation retrieval followed by code implementation. **Context7** is an MCP (Model Context Protocol) server that dynamically injects up-to-date, version-specific documentation and code examples into your AI assistant's context. It ensures that code suggestions and implementations are based on the latest official documentation, reducing errors from outdated or hallucinated APIs. [Learn more about Context7](https://apidog.com/blog/context7-mcp-server/) | `get-library-docs` → transform → `write_file` |
...
| **Playwright → Desktop Commander** | Browser interaction results saved to filesystem. **Playwright** is a fast and reliable end-to-end testing and browser automation tool for modern web apps. It supports all major browsers and platforms, allowing you to automate user interactions, capture screenshots, and verify UI behavior. Playwright is widely used for automated testing and can be integrated into CI/CD workflows. [Learn more about Playwright](https://playwright.dev/) | `browser_snapshot` → transform → `write_file` |
